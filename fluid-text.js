...
if (!('gpu' in navigator)) {
  ...
  window.__tsubasaFluid = { engine:'three-fluid-fx', renderer:'static-fallback', pointer:false, reason:'WebGPU unavailable' };
} else {
  const CAMERA_FOV = 45;
  const CAMERA_Z = 6.4;
  const FIXED_FLUID_DT = 1 / 60;
  const MAX_FLUID_SUBSTEPS = 2;
  ...
  try {
    ...
    const bgMesh = new Mesh(new PlaneGeometry(1,1), bgMaterial); scene.add(bgMesh);
    const logoMaterial = new MeshBasicMaterial({ map: logoTexture, transparent:true, depthWrite:false, toneMapped:false });
    const logoMesh = new Mesh(new PlaneGeometry(1,1), logoMaterial); logoMesh.position.z=.12; scene.add(logoMesh);
    ...
    const distortionIntensity=uniform(.52), overlayIntensity=uniform(.68), overlayOpacity=uniform(.38), overlayVelocityScale=uniform(1), elapsedTime=uniform(0), dyeTexel=uniform(new Vector2(1/512,1/512)), cursorColor=uniform(new Color(.88,.95,1)), vibrance=uniform(.42);
    ...
    window.__tsubasaFluid = { engine:'three-fluid-fx', renderer:'static-fallback', pointer:false, reason:String(error?.message||error) };
  }
}
